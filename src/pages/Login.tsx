...
const Login = () => {
  ...
  const handleSocialLogin = async (provider: "google" | "github") => {
    try {
      ...
      if (error instanceof Error) {
        const errorMessage = `Failed to sign in with ${provider}. ${error.message}`;
        setLoginError(errorMessage);
        toast.error(errorMessage);
      } else {
        const errorMessage = `An unexpected error occurred while signing in with ${provider}. Please try again.`;
        setLoginError(errorMessage);
        toast.error(errorMessage);
      }
      setLoading(false);
